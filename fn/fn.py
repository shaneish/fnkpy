...
class Fn:
    ...
    def map(self):
        output_collection = self.args.output_collection(map(self.args.func, self.parsed_expr))
        self._stdout(output_collection)

    def filter(self):
        output_collection = self.args.output_collection(filter(self.args.func, self.parsed_expr))
        self._stdout(output_collection)
    ...
    def _parse_expr(self) -> set | list | dict | tuple:
        ...
        self.args.collection_type([self.args.data_type(entry) for entry in the_iterable])
    ...
    def _load_imports(self):
        for import in self.args.import:
            exec(f"import {import}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--separator", type = str | None, default = None, help = "String to separate both input and output data.  If specified, overrides both `--separator_in` and `--separator_out`.")
